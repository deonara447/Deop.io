...
namespace Deop.io
{
    public partial class GameScreen : UserControl
    {
        ...
        public void OnStart()
        {
            ...
            p1 = new Bot(blueBrush, 1, "basic", 50, 430, 40, baseMaxHealth, baseDamage, baseSpeed, "none", baseBulletHealth, baseBulletDamage, baseBulletSpeed, baseReload, baseRegen, baseMaxHealth, 1, 0, 0);
            botList.Add(p1);
            p2 = new Bot(redBrush, 2, "basic", 1510, 430, 40, baseMaxHealth, baseDamage, baseSpeed, "none", baseBulletHealth, baseBulletDamage, baseBulletSpeed, baseReload, baseRegen, baseMaxHealth, 1, 0, 0);
            botList.Add(p2);
        ...
        private void gameLoop_Tick(object sender, EventArgs e)
        {
            ...
            else if (randGen.Next(1, squareGenerationSpeed) == 12)
            {
                ...
                if (addShape == true)
                {
                    Triangle t = new Triangle(triangleX, triangleY, triangleSize, triangleHp, triangleDamage, 2, "none");
                    triangleList.Add(t);
                }
            }
            ...
            if (healthRegenCounter == 100)
            {
                ...
                {
                    if (p1.hp > p1.maxHealth - p1.healthRegen)
                    {
                        p1.hp = p1.maxHealth;
                    }
                    else 
                    { 
                        p1.hp += p1.healthRegen;
                    }
                }
                if (p2.hp < p2.maxHealth)
                {
                    if (p2.hp > p2.maxHealth - p2.healthRegen)
                    {
                        p2.hp = p2.maxHealth;
                    }
                    else
                    {
                        p2.hp += p2.healthRegen;
                    }
                }
                healthRegenCounter = 0;
            }
